...
| Privacy tooling and local intelligence helper pass | Codex | Done | RapidFuzz added as optional helper with fallback; Presidio/small-text/theme/structlog deferred; privacy-safe helpers and tests added; full suite passed. |
| Auth outage fallback repair | Codex | Done | Source auth now falls back to verified local users only for Supabase network/DNS/timeout failures; local EXE rebuilt and health-smoked. |
