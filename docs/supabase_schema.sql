...
CREATE TABLE IF NOT EXISTS kyc_settings (
    ...
    escalation_after_missed     INTEGER DEFAULT 2,
    escalation_recipients       JSONB DEFAULT '[]'::jsonb,
    last_inspection_at          TIMESTAMPTZ,
    phone_team_members          JSONB DEFAULT '[]'::jsonb,
    remember_account            BOOLEAN DEFAULT FALSE,
    account_alias               TEXT,
    app_version                 TEXT DEFAULT '0.1.3',
    updated_at                  TIMESTAMPTZ DEFAULT NOW(),
    CHECK (id = 1)
);
...
CREATE TABLE IF NOT EXISTS kyc_inspection_events (
    ...
    status              TEXT DEFAULT 'pending'
        CHECK (status IN ('pending', 'acknowledged', 'snoozed', 'completed', 'skipped', 'expired')),
...
CREATE TABLE IF NOT EXISTS kyc_acknowledgements (
    id                  BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    event_id            BIGINT REFERENCES kyc_inspection_events(id) ON DELETE CASCADE,
    action              TEXT NOT NULL CHECK (action IN ('acknowledge', 'snooze', 'complete', 'skip')),
    reason              TEXT,
...
CREATE TABLE IF NOT EXISTS kyc_audit_log (
    id                  BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    event_id            BIGINT REFERENCES kyc_inspection_events(id) ON DELETE SET NULL,
...
ALTER TABLE kyc_inspection_events ENABLE ROW LEVEL SECURITY;
ALTER TABLE kyc_acknowledgements ENABLE ROW LEVEL SECURITY;
...
DROP POLICY IF EXISTS "allow_read_kyc_settings"         ON kyc_settings;
DROP POLICY IF EXISTS "allow_upsert_kyc_settings"       ON kyc_settings;
DROP POLICY IF EXISTS "allow_update_kyc_settings"       ON kyc_settings;
DROP POLICY IF EXISTS "allow_read_kyc_events"           ON kyc_inspection_events;
DROP POLICY IF EXISTS "allow_insert_kyc_events"         ON kyc_inspection_events;
DROP POLICY IF EXISTS "allow_update_kyc_events"         ON kyc_inspection_events;
DROP POLICY IF EXISTS "allow_insert_kyc_ack"            ON kyc_acknowledgements;
DROP POLICY IF EXISTS "allow_insert_kyc_audit"          ON kyc_audit_log;
...
-- KYC operational module sync. These policies let the desktop app mirror
-- non-secret KYC configuration and event history through the publishable key.
CREATE POLICY "allow_read_kyc_settings"
    ON kyc_settings FOR SELECT TO anon USING (true);
CREATE POLICY "allow_upsert_kyc_settings"
    ON kyc_settings FOR INSERT TO anon WITH CHECK (true);
CREATE POLICY "allow_update_kyc_settings"
    ON kyc_settings FOR UPDATE TO anon USING (true) WITH CHECK (true);
CREATE POLICY "allow_read_kyc_events"
    ON kyc_inspection_events FOR SELECT TO anon USING (true);
CREATE POLICY "allow_insert_kyc_events"
    ON kyc_inspection_events FOR INSERT TO anon WITH CHECK (true);
CREATE POLICY "allow_update_kyc_events"
    ON kyc_inspection_events FOR UPDATE TO anon USING (true) WITH CHECK (true);
CREATE POLICY "allow_insert_kyc_ack"
    ON kyc_acknowledgements FOR INSERT TO anon WITH CHECK (true);
CREATE POLICY "allow_insert_kyc_audit"
    ON kyc_audit_log FOR INSERT TO anon WITH CHECK (true);
