...
@contextmanager
def _configured_client(tmp_path: Path, monkeypatch: pytest.MonkeyPatch, *, admin_exists: bool) -> Iterator[TestClient]:
    ...
    monkeypatch.setattr(main, "admin_setup_available", lambda: True)
...
def test_login_redirects_to_first_run_setup_when_no_admin(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_api_first_run_setup_refuses_when_admin_exists(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
